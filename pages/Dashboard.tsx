...
const Dashboard = () => {
  ...
  const chartData = expenses.reduce((acc: any[], curr) => {
    const date = new Date(curr.date).toLocaleDateString('pl-PL', { day: '2-digit', month: 'short' });
    ...
    return acc;
  }, []).sort((a, b) => a.name.localeCompare(b.name)).slice(-7); // Last 7 entries
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Wydano Łącznie</CardTitle>
            <CreditCard className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatMoney(totalSpent, budget.currency)}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Pozostały Budżet</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatMoney(budget.totalLimit, budget.currency)}</div>
            <Link to="/settings" className="text-xs text-primary underline mt-1 block">
              Zmień limit
            </Link>
          </CardContent>
        </Card>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: number) => formatMoney(value, budget.currency)}
                  />
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
             <div className="space-y-4">
               {recentExpenses.length === 0 && <p className="text-sm text-muted-foreground">Brak wydatków.</p>}
