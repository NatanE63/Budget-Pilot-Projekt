...
const step2Schema = z.object({
  category: z.enum(['Restauracje', 'Atrakcje', 'Transport', 'Nocleg', 'Zakupy', 'Inne'] as const),
...
type FormData = z.infer<typeof finalSchema> & { confirmed?: boolean };
...
const AddExpense = () => {
  ...
  const { formState: { errors, isValid }, trigger, control, getValues, register } = form;
    ...
    <div className="max-w-xl mx-auto py-10 animate-fade-in">
      <Card>
        <div className="p-6 border-b">
           <h2 className="text-2xl font-bold mb-6 text-center">Dodaj Nowy Wydatek</h2>
            ...
            {currentStep === 0 && (
              <div className="space-y-4 animate-fade-in">
                <div className="space-y-2">
                  ...
                  <Input {...register('title')} placeholder="np. Obiad w pizzerii" autoFocus />
            ...
            {currentStep === 1 && (
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="space-y-2">
                  ...
                  <Input {...register('location')} placeholder="np. Kraków, Rynek" />
                </div>

                <div className="space-y-2">
                   <Label>Opis (Opcjonalnie)</Label>
                   <Input {...register('description')} placeholder="Dodatkowe notatki..." />
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fade-in text-center">
                <div className="bg-muted p-6 rounded-lg space-y-3">
                   <h3 className="text-lg font-medium text-foreground">Podsumowanie</h3>
                   ...
                   <div className="text-sm text-muted-foreground text-left space-y-1 pl-4 border-l-2 border-primary">
                     <p><strong>Co:</strong> {getValues('title')}</p>
                     <p><strong>Kategoria:</strong> {getValues('category')}</p>
                     <p><strong>Data:</strong> {getValues('date')}</p>
                     {getValues('location') && <p><strong>Gdzie:</strong> {getValues('location')}</p>}
                   </div>
                </div>
                <p className="text-sm text-muted-foreground">Czy wszystko się zgadza?</p>
              </div>
            )}
