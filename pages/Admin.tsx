...
const COLORS = ['#FF8042', '#0088FE', '#8884d8', '#FFBB28', '#00C49F', '#ffc658'];
              ...
              {categoryData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(val: number) => formatMoney(val, budget.currency)} />
                    <Legend />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
           ...
           <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center border-b pb-2">
                 ...
                 <span className="font-bold">
                   {expenses.length > 0 ? formatMoney(expenses.reduce((a,b) => a+b.amount,0)/expenses.length, budget.currency) : '0,00 zł'}
                 </span>
              </div>
              <div className="flex justify-between items-center border-b pb-2">
                 <span className="text-muted-foreground">Najdroższa kategoria</span>
                 <span className="font-bold">
            ...
            <tbody>
              {expenses.map(e => (
                <tr key={e.id} className="border-b last:border-0 hover:bg-muted/50">
                  ...
                  <td className="p-3 text-right font-medium">{formatMoney(e.amount, budget.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
